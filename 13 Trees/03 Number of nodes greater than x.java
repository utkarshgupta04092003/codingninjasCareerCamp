...
Input format :
Single Line : First Integer denotes x and rest of the elements in level order form 
separated by space. Order is - 
 ...
 * class TreeNode<T> {
	...
	public static int numNodeGreater(TreeNode<Integer> root,int x){
        ...
        for(int i=0;i<root.children.size();i++){
            ...
            childNumNodes += numNodeGreater(root.children.get(i),x);
            
            
        }
        if(root.data > x){
            return childNumNodes +1;
        }
